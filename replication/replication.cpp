...
void naruto::replica::Replication::initializer() {
    master_host_ = "";
    master_port_ = 0;
    dirty_ = 0;
    cronloops_ = 0;
...
void naruto::replica::Replication::_init() {
...
const std::string &naruto::replica::Replication::getMasterHost() const { return master_host_; }

void naruto::replica::Replication::setMasterHost(const std::string &masterHost) { master_host_ = masterHost; }

int naruto::replica::Replication::getMasterPort() const { return master_port_; }

void naruto::replica::Replication::setMasterPort(int masterPort) { master_port_ = masterPort; }

bool naruto::replica::Replication::isIsMaster() const { return is_master_; }

void naruto::replica::Replication::setIsMaster(bool isMaster) { is_master_ = isMaster; }

naruto::replica::replState naruto::replica::Replication::getReplState() const { return repl_state_; }

void naruto::replica::Replication::setReplState(naruto::replica::replState repl_state) { repl_state_ = repl_state; }

naruto::replica::replConf& naruto::replica::Replication::getReplConf() { return repl_conf_;}

void naruto::replica::Replication::_remove_bgsave_tmp_file(pid_t childpid) {
    std::string tmpfile = _bgsave_file_name(childpid);
    unlink(tmpfile.c_str());
}

void naruto::replica::Replication::addSlave(naruto::replica::replicaLink *nc) {
    if (nc == nullptr) return;
    slaves_.push_back(nc);
}

void naruto::replica::Replication::stop() {
...
void naruto::replica::Replication::onReplCron(ev::timer& watcher, int event) {
    cronloops_++;
...
//    LOG(INFO) << "onReplCron...5";
    ...
    // 断开超时从服务器
    slaves_.remove_if([this](replicaLink* nc){
    ...
    if ((cronloops_ % repl_ping_slave_period_) == 0 && !slaves_.empty()){
        ...
        for (const auto& slave : slaves_){
            ...
            slave->sendMsg(ping, cmdtype::REPL_PING);
        }
    }
...
void naruto::replica::Replication::replConfFlush() {
...
void naruto::replica::Replication::onReplConfFlushCron(ev::timer& watcher, int event) { replConfFlush();}

void naruto::replica::Replication::onBgsave(ev::timer& watcher, int event) { bgsave(); }

void naruto::replica::Replication::bgsave() {
...
void naruto::replica::Replication::onBgsaveFinish(ev::child& child, int events) {
    if (bgsave_child_pid_ == -1) return;
...
void naruto::replica::Replication::onReadSyncBulkPayload(ev::io& watcher, int event) {
    char buf[4096];
    ...
    if (repl_transfer_read_ == repl_transfer_size_){
        ...
        master_->flag |= (unsigned)replicaLink::flags::MASTER;
        repl_state_ = replState::CONNECTED;

        // 安装读事件开始接受增量消息
        master_->repl_rio = std::make_shared<ev::io>();
        master_->repl_rio->set<replicaLink, &replicaLink::onRead>(master_.get());
...
void naruto::replica::Replication::onSyncWithMaster(ev::io & watcher, int event) {
    ...
    if (repl_state_ == replState::CONNECTING){
        ...
        replication::command_ping ping;
        ping.set_ip("1111");
        master_->write(ping, cmdtype::REPL_PING);
...
void naruto::replica::Replication::backlogFeed(int tid, const unsigned char *s, size_t n, uint16_t type) {
...
void naruto::replica::Replication::backlogFeed(int tid, const ::google::protobuf::Message & msg, uint16_t type) {
    auto repl = repl_[repl_pos_.load()];
...
void naruto::replica::Replication::backlogFeed(int tid, utils::Bytes& pack) {
    auto repl = repl_[repl_pos_.load()];
    // 写到复制积压缓冲区
    auto& local_repl = (*repl)[tid];
    auto command_id = repl_command_incr_.load();
...
void naruto::replica::Replication::_backlog_feed(const unsigned char* data, size_t size) {
...
void naruto::replica::Replication::_backlog_flush() {
...
void naruto::replica::Replication::_merge_backlog_feed_slaves() {
...
void naruto::replica::Replication::_undo_connect_master() {
...
void naruto::replica::Replication::_connect_master() {
    ...
    // 连接并监听主服务的读写事件
    master_ = std::make_shared<replicaLink>(ops);
    master_->flag |= (unsigned)replicaLink::flags::MASTER;
...
// 停止下载 RDB 文件
void naruto::replica::Replication::_abort_sync_transfer() {
...
void naruto::replica::Replication::_heartbeat() {
    if (!master_ || master_->connect->broken()) return;
    LOG(INFO) << "heartbeat.......";
    replication::command_ping ping;
    ping.set_ip("127.0.0.1");
    master_->sendMsg(ping , cmdtype::REPL_PING);
}

int naruto::replica::Replication::_slave_try_partial_resynchronization(int fd) {
    ...
    try {
        master_->sendMsg(psync, cmdtype::REPL_PSYNC, reply);
...
void naruto::replica::Replication::cacheMaster() {
    if (!master_) return;
...
void naruto::replica::Replication::statSlave() {
    if (slaves_.empty()){
        repl_no_slaves_since_ = std::chrono::system_clock::now();
    }
}

void naruto::replica::Replication::discardCacheMaster() {
...
std::string naruto::replica::Replication::_bgsave_file_name(pid_t pid) {
    return FLAGS_repl_dir + "/tmp-" + std::to_string(pid) + ".db";
}

std::shared_ptr<naruto::replica::Replication> naruto::replica::replptr = std::make_shared<naruto::replica::Replication>();